# Kamailio SIP Server v4.2.x (stable): Pseudo-Variables
...
Assigning $null to the uri attribute will drop the branch, for the rest
of attributes will just set the value to null.
...
in configuration file (will add a new branch).\</fc>
...
\<fc #0000ff>It is a R/W variable (you can assign values to it directly
in configuration file). Transport proto can be omitted when assigning
...
It is a R/W variable (you can assign values to it directly in
configuration file).
...
\<fc #0000ff>It is a R/W variable.\</fc>
...
\* line - return current line in config \* name - return the name of
current config file
