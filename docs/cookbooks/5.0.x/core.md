# Kamailio SIP Server v5.0.x (stable): Core Cookbook
...
The port advertised in Via header and other destination lumps. If empty
or not set (default value) the port from where the message will be sent
is used. Same warnings as for 'advertised_address'.
...
evaluated at runtime (see log_prefix_mode about when/how evaluation is
done).
...
If set to 0 (default), then log_prefix is evaluated when the sip message
...
changed at runtime via RPC commands, without restarting Kamailio. See
cfg_rpc module for RPC details.
